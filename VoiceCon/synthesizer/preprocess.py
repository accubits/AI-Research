...
def create_embeddings_metadata(synthesizer_root: Path, encoder_model_fpath: Path, n_processes: int):
...
def create_embeddings(audio_seq_path: Path, speaker: str, synthesizer_root: Path, encoder_model_fpath: Path, n_processes: int):
    embed_dir = synthesizer_root.joinpath("embeds/%s" % speaker)
    embed_dir.mkdir(parents=True, exist_ok=True)

    # Gather the input wave filepath and the target output embed filepath
    fpaths = [(file_path[1], embed_dir.joinpath(file_path[1].split("/")[-1].split(".")[0]+".npy"), file_path[0]) for file_path in audio_seq_path]
